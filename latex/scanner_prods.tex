...
% the CTR findings/calibration, and the deliverables for CryspBrainSim.
...
The note closes with the production deliverables (section~\ref{sec:deliverables}): two
ten-shard list-mode masters --- BGO 195\,K and cryogenic CsI on the same $2X_0$ ring, from the
identical proton-activity source --- with the geometry, statistics, file layout and column
semantics the downstream \textsc{CryspBrainSim} analysis consumes.
\end{abstract}
...
Two ten-shard list-mode masters are produced, one per representative scanner, into the
\texttt{PtCryspProds/} tree (layout contract: \texttt{PRODUCTS.md} = the tree's
\texttt{README.md}; column schema: \texttt{SCHEMA.md} at the tree root, refreshed on every
publish, as is this note's PDF):

\begin{center}
\begin{tabular}{ll}
\toprule
BGO 195\,K & \texttt{uniform\_headep\_sobp\_1e8/crysp\_ring\_1m\_bgo\_2x0/bgo\_195k/fast\_1Gy/} \\
CsI        & \texttt{uniform\_headep\_sobp\_1e8/crysp\_ring\_1m\_csi\_2x0/csi/fast\_1Gy/} \\
\bottomrule
\end{tabular}
\end{center}

Each leaf holds \texttt{lors\_shard000.h5}\,\ldots\,\texttt{lors\_shard009.h5} plus
\texttt{config.toml} (the regeneration recipe); \texttt{scanner\_geometry.json} sits at the
scanner level, and the shared \texttt{phantom/} ($\mu$-map inputs) and \texttt{truth/}
(dose-$R_{80}$ + activity-$R_{50}$) bundles at the scenario level.

\subsection*{Scanner geometry}

Both scanners are the same closed cylindrical ring, differing only in crystal and depth:
...
\toprule
 & ring (shared) & \\
\midrule
inner radius            & \multicolumn{2}{c}{$387$\,mm} \\
axial length            & \multicolumn{2}{c}{$1024$\,mm ($\pm 512$\,mm)} \\
segmentation            & \multicolumn{2}{c}{$48$ blocks in $\varphi$ $\times$ $20$ wheels in $z$} \\
block transverse size   & \multicolumn{2}{c}{$50.6$\,mm (arc at inner radius) $\times$ $51.2$\,mm ($z$)} \\
\midrule
 & BGO 195\,K & CsI \\
\midrule
crystal depth ($2X_0$)  & $22.36$\,mm & $37.20$\,mm \\
outer radius            & $409.4$\,mm & $424.2$\,mm \\
\bottomrule
\end{tabular}
\end{center}

The phantom is the scenario's: a uniform G4\_BRAIN\_ICRP ellipsoid (semi-axes
$7.2 \times 8.7 \times 10.2$\,cm, centred at $(0, -3, 0)$\,cm), inside an air world. Monolithic
blocks, continuous 3-D readout; the detector response (table~\ref{tab:systems}) is already
applied in the files --- do \emph{not} re-apply it.

\subsection*{Source and statistics}

The source is the frozen \texttt{uniform\_headep\_sobp\_1e8} scenario at 1\,Gy, \texttt{fast}
budget (acquisition window $[0, 1200]$\,s): ${\approx}8.02\times10^7$ decays per shard,
5 isotopes ($^{15}$O, $^{11}$C, $^{13}$N, $^{10}$C, $^{14}$O). Shard $R$ is seeded by
(\texttt{master\_seed}=1, \texttt{realization}=$R$), $R = 0\ldots9$, and \textbf{the same $R$
sees the identical annihilation set on both scanners} --- differences between the two masters
are detector-only, event by event. Expected per-shard and pooled-master statistics (measured on
shard~0):
...
\toprule
 & BGO 195\,K & CsI \\
\midrule
detector LORs per shard (\texttt{lors\_det})  & $15.3$\,M & $6.1$\,M \\
acceptance (of $8\times10^7$ annihilations)   & $19.0\%$  & $7.6\%$ \\
true / scatter / random                       & $72.3 / 27.4 / 0.3\%$ & $86.6 / 13.3 / 0.1\%$ \\
shard file size                               & ${\sim}490$\,MB & ${\sim}190$\,MB \\
pooled master ($10$ shards)                   & ${\sim}153$\,M LORs & ${\sim}61$\,M LORs \\
\bottomrule
\end{tabular}
\end{center}

The contrast is the point of the pair: BGO delivers $2.5\times$ the statistics; CsI delivers
purity (half the scatter fraction, $3\times$ fewer randoms) --- the range study arbitrates.
