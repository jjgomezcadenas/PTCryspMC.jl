...
The simulation is a fast, photon-only Monte Carlo. Its building blocks:
\begin{itemize}[nosep]
\item \textbf{Geometry} (\texttt{geometry.jl}): a Geant4-style hierarchy of solids
(\texttt{Solid}: \texttt{Cylinder}, \texttt{Box}, \texttt{CylShell}), logical volumes
(solid $+$ material) and physical volumes (a logical volume placed in the world), with
\texttt{is\_inside} and the ray entry/exit distances (\texttt{distance\_to\_entry},
\texttt{distance\_to\_exit}; the ray--solid math is in \texttt{docs/navigation.tex}). The
world is a non-interacting cylinder of air that encloses the phantom (a placed
\texttt{Cylinder}) and the ring (one \texttt{CylShell} with a $(\varphi, z)$ block/wheel
index; the plane-crossing distances are added once dead gaps return).
\item \textbf{Cross sections} (\texttt{nist\_data.jl}, \texttt{materials.jl}): the XCOM
loader and \texttt{sigma\_macro(material, E)}, which returns the macroscopic Compton,
photoelectric and pair cross sections at an energy. Tables (\SI{10}{keV}--\SI{10}{MeV}) are
in place for water (the phantom) and the CsI and BGO crystals; tissue and LYSO can be added
the same way.
\item \textbf{Transport} (\texttt{sampling.jl}, \texttt{transport.jl}): the Compton and
photoelectric samplers, the interaction kernel (\texttt{sample\_interaction}) and the
single-volume step loop (\texttt{propagate\_photon}). Only the photon path is followed,
with local energy deposition; electrons are not transported.
\item \textbf{Navigation} (\texttt{navigator.jl}): \texttt{navigate\_photon} carries a
photon \emph{across} volumes --- phantom (water) $\to$ air $\to$ ring --- switching the
material at each boundary. It reuses \texttt{propagate\_photon} for each absorbing volume
and skips straight through the non-interacting air mother, so the interaction physics and
the step loop are each written once (\texttt{docs/navigation.tex}~\S4). Its output stack
tags every interaction with its volume and, in the ring, its readout block.
\end{itemize}

\paragraph{Built and checked so far.} The geometry (air world, phantom, the
\texttt{CylShell} ring and its $(\varphi,z)$ grid), the cross sections, the transport and
the multi-volume navigation are in place and tested. Four studies exercise them:
\SI{511}{keV} photons through the water phantom reproduce Beer--Lambert attenuation;
photons fired into a single crystal (a \texttt{Box} of CsI or BGO) give its containment,
photoelectric/Compton split and interaction-depth distributions; a point source firing
back-to-back \SI{511}{keV} pairs isotropically into the ring (interior treated as air)
gives the geometric acceptance ($\sim$\SI{79.5}{\percent}, the ring's axial subtense) and
the per-crystal containment; and navigating the same pairs through the \emph{water
phantom} makes the patient-scatter background appear ($\sim$\SI{58}{\percent} of photons
Compton-scatter in \SI{8}{cm} of water, matching Beer--Lambert). Note that the
``full-energy'' fractions quoted by these studies use a \emph{truth} \SI{505}{keV} cut
(at truth a fully-absorbed photon deposits exactly \SI{511}{keV}): this isolates the
unscattered, fully-contained subset --- the both-photon full-energy fraction falls from
$\sim$0.24 (air) to $\sim$0.04 (through the phantom) for CsI, since both photons must
cross the water unscattered --- and is \emph{not} the coincidence efficiency. The real
selection smears by $\mathrm{FWHM}(E)$ and applies a $\pm2\,\mathrm{FWHM}$ energy window
(Sec.~\ref{sec:detsim}), keeping scattered photons as \emph{scatter} coincidences. What
remains: the same-annihilation coincidence output (true/scatter), then hit formation,
selection and the randoms pass.
