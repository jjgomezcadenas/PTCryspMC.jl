...
photoelectric samplers and the photon step loop (\texttt{propagate\_photon}). Only the
photon path is followed, with local energy deposition; electrons are not transported.
\end{itemize}

\paragraph{Built and checked so far.} The geometry (air world, phantom, the
\texttt{CylShell} ring and its $(\varphi,z)$ grid), the cross sections and the transport are
in place and tested. Three studies exercise them: \SI{511}{keV} photons through the water
phantom reproduce Beer--Lambert attenuation; photons fired into a single crystal (a
\texttt{Box} of CsI or BGO) give its containment, photoelectric/Compton split and
interaction-depth distributions; and a point source firing back-to-back \SI{511}{keV} pairs
isotropically into the ring gives the geometric acceptance ($\sim$\SI{79.5}{\percent}, the
ring's axial subtense) and the per-crystal containment --- the clean-coincidence fraction
(both photons fully absorbed in one crystal) is $\sim$0.29 for CsI and $\sim$0.77 for the
denser BGO. What remains: the multi-volume navigator that carries a phantom-born photon
through to the ring, then hit formation, selection and the randoms pass.
