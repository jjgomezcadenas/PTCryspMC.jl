...
(\texttt{Solid}: \texttt{Cylinder}, \texttt{Box}, and the coming \texttt{CylShell}), logical volumes
...
\texttt{distance\_to\_exit}). The phantom is a placed \texttt{Cylinder}; the ring is one
\texttt{CylShell} with a $(\varphi, z)$ block index and the corresponding plane-crossing
distances added on top --- the one piece of new geometry.
...
\paragraph{Built and checked so far.} The geometry, cross sections and transport are in
place and tested. Two studies exercise them: \SI{511}{keV} photons through the water phantom
reproduce Beer--Lambert attenuation, and 511\,keV photons fired into a single crystal (a
\texttt{Box} of CsI or BGO) give its energy containment, photoelectric/Compton split and
interaction-depth distributions --- characterising the crystal response before the full
ring. Source injection, the block grid, hit formation, selection and the randoms pass remain.
