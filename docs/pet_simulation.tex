...
the per-crystal containment --- the clean-coincidence fraction (both photons fully absorbed
in one crystal) $\sim$0.29 for CsI, $\sim$0.77 for the denser BGO; and navigating the same
pairs through the \emph{water phantom} makes the patient-scatter background appear
($\sim$\SI{58}{\percent} of photons Compton-scatter in \SI{8}{cm} of water, matching
Beer--Lambert) and drops the clean-coincidence fraction to $\sim$0.04 (CsI) --- both
photons must now cross the phantom unscattered. What remains: the same-annihilation
coincidence output (true/scatter), then hit formation, selection and the randoms pass.
